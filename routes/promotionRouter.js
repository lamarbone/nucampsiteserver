...
.delete((req, res, next) => {
    ...
    .then(promotion => {
        if (promotion) {
        for ( let i = (promotion.s.length-1); i >= 0; i--) {
            promotion.s.id(promotion.s[i]._id).remove();
        }
        promotion.save()
        .then(promotion => {
        res.statusCode = 200;
...
promotionRouter.route('/:promotionId/s/:Id')
.get((req, res, next) => {
    Promotion.findById(req.params.promotionId)
    .then(promotion => {
